...
def __last_candles() -> bool:
    ...
    with open(path, 'r') as file:
            ...
            if last_candle:              # Range <= 200 USD                   # 0.1% far from MA100
                range_ok = True if abs(open_price - last_min) <= 200 and abs(open_price - close_mean) / close_mean >= 0.1/100 else False
            else:
                range_ok = True if abs(open_price - last_max) <= 200 and abs(open_price - close_mean) / close_mean >= 0.1/100 else False
